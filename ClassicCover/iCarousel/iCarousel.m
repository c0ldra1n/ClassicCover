...
BOOL pointIsInBoundary(CGSize size, CGPoint point){
    ...
    if (point.x < 0 || point.y < 0) {
        ...
        if(size.width - point.x < 0 || size.height - point.y < 0){
            return NO;
        }
        return YES;
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gesture shouldReceiveTouch:(UITouch *)touch
{
    ...
    if(!_tapShortCutEnabled){
        
        if(!pointIsInBoundary(self.currentItemView.frame.size, [touch locationInView:self.currentItemView])){
            [(CRCCAlbumArtView *)self.currentItemView flip:NO];
        }
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    if (index != NSNotFound)
    {
        ...
        if (!_delegate || [_delegate carousel:self shouldSelectItemAtIndex:index withTap:tapGesture])
        {
